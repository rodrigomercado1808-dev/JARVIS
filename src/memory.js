...
class MemoryManager {
  ...
  async saveMemory({ userId, content, type = 'general', importance = 0.5, metadata = {} }) {
    ...
    if (col) { try { await col.doc(id).set({ id, type, importance, createdAt: item.createdAt, encrypted: await seal(item) }); return { ...item, storage: 'firebase' }; } catch { return { ...item, storage: 'sessionStorage' }; } }
    return { ...item, storage: 'sessionStorage' };
  }
  async records(userId) { const col = this.collection(userId); if (!col) return []; try { const snap = await col.orderBy('createdAt', 'desc').limit(100).get(); return snap.docs.map(d => d.data()); } catch { return []; } }
