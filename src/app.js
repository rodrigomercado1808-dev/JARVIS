...
const { inspect, capabilities, persona } = require('./assistant-orchestrator');
...
function buildApp() {
  ...
  app.post('/api/chat', async (request, reply) => {
    ...
    const user = await resolveUser(request); if (!user) return reply.code(401).send({ success: false, error: 'authentication_required' }); const { message, history } = parsed.data; const plan = inspect(message, history); const context = await memory.getContext(user.uid, message); let webResults = []; let usedTools = ['orchestrator'];
    if (plan.mode === 'capabilities') return { success: true, response: capabilities(), conversation_id: `conversation:${user.uid}`, message_id: crypto.randomUUID(), used_tools: usedTools, storage: user.authenticated && isConnected() ? 'firebase' : 'sessionStorage', authenticated: user.authenticated, learned: false, domain: 'general', intent: 'capabilities', confidence: 1 };
    if (plan.mode === 'datetime') return { success: true, response: `La fecha y hora del servidor es ${new Date().toLocaleString('es-AR', { dateStyle: 'full', timeStyle: 'medium' })}.`, conversation_id: `conversation:${user.uid}`, message_id: crypto.randomUUID(), used_tools: [...usedTools, 'datetime'], storage: user.authenticated && isConnected() ? 'firebase' : 'sessionStorage', authenticated: user.authenticated, learned: false, domain: 'general', intent: 'datetime', confidence: 1 };
    if (plan.mode === 'calculator') { const expression = message.replace(/^(cuanto es|cuánto es|calcula|calcular)\s*/i, ''); try { const result = await tools.calculator.execute({ expression }); return { success: true, response: `El resultado es ${result.result}.`, conversation_id: `conversation:${user.uid}`, message_id: crypto.randomUUID(), used_tools: [...usedTools, 'calculator'], storage: user.authenticated && isConnected() ? 'firebase' : 'sessionStorage', authenticated: user.authenticated, learned: false, domain: 'general', intent: 'calculator', confidence: 1 }; } catch { return reply.code(400).send({ success: false, error: 'calculation_not_allowed' }); } }
    if (plan.mode === 'safe-action') return { success: true, response: 'Puedo ayudarte con esa acción, pero antes necesito que me indiques exactamente qué recurso querés modificar. No ejecuto acciones destructivas o ambiguas.', conversation_id: `conversation:${user.uid}`, message_id: crypto.randomUUID(), used_tools: [...usedTools, 'safety-guard'], storage: user.authenticated && isConnected() ? 'firebase' : 'sessionStorage', authenticated: user.authenticated, learned: false, domain: 'general', intent: 'safe-action', confidence: 1 };
    ...
    const result = await answer({ message, history, memories: context.memories, webResults }); usedTools.push(...result.usedTools); if (user.authenticated && isConnected()) await memory.saveMemory({ userId: user.uid, content: `Pregunta: ${message}\nRespuesta: ${result.text.slice(0, 1800)}`, type: 'conversation_learning', importance: 0.35, metadata: { domain: result.domain, intent: result.intent } }); return { success: true, response: persona(result.text, plan), conversation_id: `conversation:${user.uid}`, message_id: crypto.randomUUID(), used_tools: [...new Set(usedTools)], storage: user.authenticated && isConnected() ? 'firebase' : 'sessionStorage', authenticated: user.authenticated, learned: webResults.length > 0, domain: result.domain, intent: result.intent, confidence: result.confidence };
  });
