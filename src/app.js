...
function buildApp() {
  ...
  app.post('/api/chat', async (request, reply) => { const parsed = z.object({ user_id: z.string().min(1).default(config.defaultUserId), message: z.string().min(1).max(10000) }).safeParse(request.body); if (!parsed.success) return reply.code(400).send({ success: false, error: parsed.error.flatten() }); const { user_id, message } = parsed.data; const context = await memory.getContext(user_id, message); let response; let usedTools = ['memory']; let storage = isConnected() ? 'firebase' : 'sessionStorage'; if (context.memories.length) { response = `Encontré esta información guardada:\n\n${context.memories.map(m => `• ${m.content}`).join('\n\n')}`; } else { try { const result = await searchWeb(message); usedTools.push('web'); if (result.results.length) { response = `No tenía esa información guardada. Encontré estas fuentes en Internet:\n\n${result.results.map((r, i) => `${i + 1}. ${r.title}\n${r.snippet}\n${r.url}`).join('\n\n')}`; const savedMemory = await memory.saveMemory({ userId: user_id, content: JSON.stringify(result), type: 'web_research', importance: 0.6, metadata: { query: message } }); storage = savedMemory.storage; } else response = 'No encontré resultados confiables para esa consulta.'; } catch (error) { response = `No encontré la respuesta en la memoria y la búsqueda web falló: ${error.message}`; } } return { success: true, response, conversation_id: `conversation:${user_id}`, message_id: crypto.randomUUID(), used_tools: usedTools, storage }; });
